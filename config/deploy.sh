...
echo "5. remotely perform <git pull;git checkout $1> on all instances of autoscaling group asg_tourfilter_web"
echo "6. run <bundle pack> on all remote instances"
echo "7. install crontab on first instance only."
...
let web_failures=`$TOUFILTER_HOME/config/deploy_web.sh $1`
let daemon_success=`$TOUFILTER_HOME/config/deploy_daemon.sh $1`
